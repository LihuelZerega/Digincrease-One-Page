...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'POST') {
    const { name, lastName, email, phoneNumber, derisedService, country, estimatedBudget, message } = req.body;
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <p><strong>Servicio Deseado:</strong> ${derisedService}</p>
        <p><strong>Pais:</strong> ${country}</p>
        <p><strong>Presupuesto Estimado:</strong> ${estimatedBudget}</p>
